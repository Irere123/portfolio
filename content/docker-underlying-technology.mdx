...
- A container think it as a single package that contains everything needed to run a process. For example it can container
...
- An Image is read-only meaning once you have created an image you can not change its content and you would have to build a new one for
  those changes. It's a template for creating a Docker container often an image is based on an image with some additional personal touches.
...
Isolating processes from each service and its associated processes from other services means that there is a smaller **blast radius for changes**. ie. if there is
an error the error stays in that process and does not affect other services
...
Answer is simple: Docker runs a virtual machine in your local machine when you install Docker Desktop
Docker uses `VirtualBox` this is the default virtual machine that comes with docker
...
<Callout emoji="🧐">
  A system call (aka syscall) is a computer program requesting a service from
...
There are a few types of linux kernel namespaces and every type of namespace will isolate a different set of resources on our system:
...
Now we have only scratched the surface and in the next series of this article we will look into volumes and dimystify there inner working stay tuned of more follow or DM me in X(formerly Twitter) for updates and questions
