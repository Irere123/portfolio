...
way to know more about the language than exploring algorithms in it. I mean if you use a certain language it will expose you to more
...
So Enough with the talking and let's dive into the code that showcases how Big O actually works.

1. O(1) --> Constant time

An algorithm is said to be constant time or O(1) if the value of T(n) (the complexity of the algorithm) is bounded by a value that does not depend on the `problem size` or
`input size`. ie. it will always take a known amount of time no matter what the input size is...

```rust
let added =  3000 * 2; // operation such as multiplication or addition
let div = 330 / 2;
```
